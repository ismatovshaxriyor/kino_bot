...
async def inline_query_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not q:
        ...
        if exact:
            movies = [exact]
        name_matches, _ = await search_movies(q, limit=MAX_INLINE_RESULTS)
        if exact:
...
async def inline_movie_command_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # faqat ro'yxatni ochish ham "ko'rish" sifatida hisoblanib, statistika/
    # grafiklarni buzib yuboradi.
    child_parts = await get_child_parts(movie)
    if child_parts:
